...
public class PuHuiDataImportTask implements ZZTask{
	...
	public boolean exec(Date baseDate) throws Exception {
		...
		String sql="select c.id,ca.email,ca.account,c.address,c.introduction,c.business,ca.contact,ca.sex,c.name,ca.mobile,ca.tel,c.address from company c left join company_account ca on ca.company_id=c.id " +
		...
		DBUtils.select(DB_AST, sql, new IReadDataHandler() {
			...
			public void handleRead(ResultSet rs) throws SQLException {
				while (rs.next()) {
					...
					map.put("address", rs.getString(12));
					list.add(map);
				}
			}
		});
		for(Map<String, Object> company:list){
			...
			saveToKL(cid,email,account,membershipCode,business,contact,sex,name,mobile,tel,
				numLogin,gmtLastLogin,industryCode,domain,isActive,registFlag,address);					
			}
		return false;
	}
	...
	private void saveToKL(Integer cid,String email, String account, String membershipCode,
			...
			Integer registFlag,String address) {
		String sql="insert into company(account,company_name,membership_code,sex,contact,mobile,address,email,tel,business,num_login,gmt_last_login,is_active,domain,industry_code,regist_flag,show_time,gmt_created,gmt_modified)values('"+account+"','"+name+"','"+membershipCode+"',"+sex+",'"+contact+"','"+mobile+"','"+address+"','"+email+"','"+tel+"','"+business+"','"+numLogin+"','"+gmtLastLogin+"','"+isActive+"','"+domain+"','"+industryCode+"','"+registFlag+"',now(),now(),now())";
