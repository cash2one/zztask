	...
	private void selectKLCompanyIdAndImport(Integer companyId) {
		...
		if(counts[0]!=0){
			updateCompanyInsert(email,account,introduction,"10051000",business,contact, sex, companyName,mobile,tel, 1, 
				DateUtil.toString(new Date(),"yyyy-MM-dd HH:mm:ss"),"1000", website,0,regesiterFlag,password, companyId);
		}else{
...
//	}
	...
	public static void main(String[] args) throws Exception {
		...
		PuHuiTask obj = new PuHuiTask();
		Date date = DateUtil.getDate("2012-08-17", "yyyy-MM-dd");
		obj.exec(date);
		}
