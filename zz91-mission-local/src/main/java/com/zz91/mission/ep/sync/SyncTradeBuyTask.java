...
public class SyncTradeBuyTask implements ZZTask {
	...
	public boolean exec(Date baseDate) throws Exception {
		...
		//取得最大的id号 和最大的更新时间
		String maxInfo=HttpUtils.getInstance().httpGet(SYNC_URL_SEED+"?table=trade_buy", HttpUtils.CHARSET_UTF8);
		...
		do{
			...
			NameValuePair[] param=new NameValuePair[]{
				new NameValuePair("tradeBuy", JSONArray.fromObject(list).toString())
				};
		...
		DBUtils.select(DB_EP, sql.toString(), new IReadDataHandler() {
			...
			public void handleRead(ResultSet rs) throws SQLException {
				...
				while(rs.next()){
					...
					buy.setGmtConfirm(rs.getDate("gmt_confirm"));
					buy.setGmtReceive(rs.getDate("gmt_receive"));
					buy.setGmtPublish(rs.getDate("gmt_publish"));
					buy.setGmtRefresh(rs.getDate("gmt_refresh"));
					buy.setValidDays(rs.getShort("valid_days"));
					buy.setGmtExpired(rs.getDate("gmt_expired"));
					...
					buy.setGmtCheck(rs.getDate("gmt_check"));
					buy.setGmtCreated(rs.getDate("gmt_created"));
					buy.setGmtModified(rs.getDate("gmt_modified"));
					list.add(buy);
				}
			}
		});
